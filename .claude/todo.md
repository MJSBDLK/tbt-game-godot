...
- [~] 4. Since we added the arrow + phantom effect for displacement moves, should we use the same system when previewing a move with the move beacons? (FIRST CUT 2026-08-21 lives on branch `rqd--move-preview-ghost` (not merged — eyeball-gated): beacons stay the path, a shared `UnitGhost` projection parks on the plan's last waypoint for player units. See that branch's todo note for the full shape. Squash-merge once seen in a build.)
  - [~] 4A. need to decide if we hold off on actually moving the unit (just show the static/fuzzy phantom preview) to the spot before committing an action - would be a departure from current design but more accurate. We should solve the problem both ways and playtest both, and see what players prefer/find less confusing. (UNPARKED 2026-08-31 after the talk. The fog objection RESOLVED in reverse: ACT_THEN_WALK is the only commit model fog can ever work with — planning is a ghost and reveals nothing, commit is the one irreversible act — so building it forecloses nothing; fog itself is filed as a rare post-alpha mission modifier in §9. BUILDING NOW on branch `rqd--move-commit-mode` (stacked on `rqd--move-preview-ghost` for UnitGhost): `Settings.move_commit_mode { WALK_THEN_ACT (today, default), ACT_THEN_WALK }` playtest toggle per the original both-ways instinct; cheap faithful variant — logic moves exactly as now, sprite/shadow/bars stay at origin with the #4 ghost on the destination until the action commits, then the walk plays and the action fires. Bonus UX win: cancel in ACT_THEN_WALK is visually clean (the sprite never moved — no teleport-back). Hint bar planning copy goes mode-aware. NOTE: fog would later need TRUE deferred logic (unit logically at origin until commit); this ships the visual variant only.)
