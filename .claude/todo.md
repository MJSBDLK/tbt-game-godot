...
- [ ] **Displacement previews** (agreed next displacement item): displacement is fully deterministic — no roll in the contest, pure geometry in the path — so show the future before the player commits. All from pure `DisplacementSystem.build_plan()` on hover/cursor-focus during targeting (chip live-paint already solved the InputSource pattern) — rendering only, zero new game logic.
