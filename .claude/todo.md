...
- [ ] We need to figure out how the game logic knows which parts of tiles are passable and by which units. For 1x1 tiles, we just need to write its properties to the JSON. easy peasy. But for stuff like a 2x2 castle, we might want the top row to be impassable to anything but fliers, while the bottom row is walkable by all units. Thoughts on how to do this cleanly? **RQD input needed — design options proposed (see conversation 2026-06-10): per-sprite terrain-rows config consulted during footprint expansion.**
