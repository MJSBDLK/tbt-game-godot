...
- [x] Lawrence wants a VICTORY screen with no information first, then the info panel slides in (from the side, top, whatever). But the first thign should be a "you won" or "you lost" with no additional information - we can repurpose the "player turn/enemy turn" banner, with some alterations, for this (Done 2026-07-06: battle end now plays the phase banner with just "VICTORY" (player blue) or "DEFEAT" (enemy red), THEN the stats panel slides down into place (bg fade + eased offset tween). Eyeball the timing/slide direction in game — distance/duration are constants in battle_result_overlay.gd.)
