...
- [ ] Tooltips: controller focus-navigation mode (Dark Souls pattern — hotkey grabs focus on tooltip-bearing icons, stick navigates, focus_entered shows themed popup, B/Back releases). Click-to-show already works on M&K and touch via TapTooltip; controller is the remaining gap.
  - [~] 2026-07-31: the BOARD half started landing — ATTACK_TARGETING now has a full keyboard/controller cursor (arrows walk valid targets, §14 brackets on the tile via TargetCursorRenderer, accept confirms, adoption doctrine mirrors the menus). Still open: board cursor for unit SELECTION + movement planning (the FE free cursor), which this deliberately did not attempt.
