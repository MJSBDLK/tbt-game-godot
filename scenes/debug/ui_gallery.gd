...
func _build_snug_rig() -> VBoxContainer:
	...
	# died — the WHOLE glyph bleaches to the ELEMENT ramp's own top (Gray 10
	# / YellowOrange 10, "all the way to white"; uniform, after the per-side
	# split read as center-vs-edge banding) and the dark ramp shadow carries
